...
#ifndef FINALPROJECT_SCROLLABLECONTAINER_CPP
#define FINALPROJECT_SCROLLABLECONTAINER_CPP
...
template<class T>
void ScrollableContainer<T>::scroll(float delta,const sf::FloatRect & bound) {
    if (!items.empty()) {
...
template<class T>
void ScrollableContainer<T>::update() {
...
template<class T>
void ScrollableContainer<T>::setPosition(sf::Vector2f pos) {
    if (!items.empty())
        for (auto *i:items) {
            i->setPosition(pos);
        }
